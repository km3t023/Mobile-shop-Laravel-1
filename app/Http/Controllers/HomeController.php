...

{
...

    {
        ...
        return redirect(route('home.addDeposit'))->with('message','Depozit dodat');
...

    {
        ...
        return redirect(route('home'))->with('message','Oglas je dodat');
    ...
    public function storeReply(Request $request)
    {
        ...
        $store_msg->reciever_id = $sender->id;
        $store_msg->ad_id = $ad->id;


        $store_msg->save();

        return redirect()->back()->with('msg','Odgovor poslat');
    ...
    public function editAd($id)
    {
        $singleAd = Ad::find($id);
        $categories = Category::all();
        return view('home.editAd', compact('categories', 'singleAd'));
    }
